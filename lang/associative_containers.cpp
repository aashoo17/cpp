...
these containers keep their elements based on some logic to their value inside the containers
they can reorder the element as per their wish for storing

map, unordered_map, multimap:
map gets ordered by its keys
unordered_map don't get ordered by keys they use hash function to put elements at some location
multimap can have same key more than once
*/
...
void map_(){
    //create
    map<int,int> a;
    map<int, string> b = {{1,"Hello"},{2,"World"}};     //using list initializer literal for map
