...
//[stl
//algorithms](https://www.cs.helsinki.fi/u/tpkarkka/alglib/k06/lectures/algorithms.html)
void algorithms() {
  // taking vector as sample here but can be used on any other sequential
  // containers - deque, list etc
  vector<int> a{100, 40, 30, 20, 50, 10, 90, 60, 80, 70};
  vector<int> b{20, 50, 10}; // this will be used to provide range of value when
                             // algorithms required
  vector<int> c{100, 40, 30, 20, 50, 10,
                90,  60, 80, 70}; // use this if write is required and don't
                                  // want to change vector a
  ...
  // so these algorithms generally return iterator - that means they kinda give
  // position search:
  // TODO: if element was not found where iterator will point - i.e. how will i
  // know element was not found search 1 element
  auto find_itr = find(cbegin, cend, 1000);
  auto find_itr2 = find_if(
      cbegin, cend, [=](int a) { return a == 10; }); // takes unary predicate
  ...
  replace_if(
      c.begin(), c.end(), [=](int a) { return a < 50; }, 7000);

  // remove elements
  // remove(begin, end, value);
  // TODO: will this remove all elements or 1 element which is first found
  // TODO: does remove and emit the new iterator as clang-tidy is saying to use
  // return value
  ...
  sort(a.begin(), a.end(), [=](int a, int b) {
    return a < b;
  }); // order may be chabged for same values
  // stable_sort() is similar but if two elements are same it will ensure that
